function [Cluster_elem,M,EW_history,W,z]=CGFFCM_ablation(X,M,k,p_init,p_max,p_step,t_max,beta_memory,N,fuzzy_degree,d,beta_z,landa,v,G,cfg)
% Ablation-enabled CGFFCM based on your CGFFCM.m
% cfg fields (logical):
%   cfg.learnW, cfg.learnZ, cfg.useMemory, cfg.adaptiveP, cfg.useV, cfg.useKernel
% defaults: all true

if nargin<18 || isempty(cfg), cfg=struct(); end
...
while true
    ...
    if cfg.learnZ
        ...
        for j=1:k
            ...
            dWkm(j,:) = (Cluster_elem(j,:).^fuzzy_degree) * dist_base;
        end
